...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CraftZones.Vest.coords, true) < 1 then
...
function BukaMenuVest()
    local elements = {}
    for item, v in pairs(Crafting.Vest) do
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'crafting_actions', {
        ...
        ESX.TriggerServerCallback('fox_simplecrafting:CekVest', function(result)
            if result then
                ...
                Citizen.Wait(35000)
                TriggerServerEvent("fox_simplecrafting:CraftingVest", CurrentCraft)
            else
