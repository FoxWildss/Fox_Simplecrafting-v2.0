...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CraftZones.Lockpick.coords, true) < 1 then
...
function BukaMenuLockpick()
    local elements = {}
    for item, v in pairs(Crafting.Lockpick) do
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'crafting_actions', {
        ...
        ESX.TriggerServerCallback('fox_simplecrafting:CekLockpick', function(result)
            if result then
                ...
                Citizen.Wait(35000)
                TriggerServerEvent("fox_simplecrafting:CraftingLockpick", CurrentCraft)
            else
